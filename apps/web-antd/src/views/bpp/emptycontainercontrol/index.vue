...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridEvents: {
    checkboxAll: handleRowCheckboxChange2,
    checkboxChange: handleRowCheckboxChange2,
  },
});
...
/** 查看子计划详情 */
const handleSubPlanDetail = async (row: EmptyContainerControlApi.subPlanVO) => {
...
/** 编辑子计划申请 */
const handleSubPlanEdit = async (row: EmptyContainerControlApi.subPlanVO) => {
  const res = await getSubPlan(row.id);
...
/** 删除子计划 */
const handleSubPlanDelete = async (row: EmptyContainerControlApi.subPlanVO) => {
            ...
            :actions="[
              {
                ...
                icon: ACTION_ICON.EDIT,
                auth: ['system:user:update'],
                onClick: handleSubPlanEdit.bind(null, row),
              },
              {
                label: '详情',
                type: 'link',
                icon: ACTION_ICON.VIEW,
                onClick: handleSubPlanDetail.bind(null, row),
              },
              {
                label: '删除',
                type: 'link',
                icon: ACTION_ICON.DELETE,
                auth: ['system:user:delete'],
                onClick: handleSubPlanDelete.bind(null, row),
                danger: true,
              },
            ]"
          />

...
import {
  ...
  SUBMIT_STATUS_COLOR,
  useLoadingGridColumns,
  useLoadingGridFormSchema,
} from "../data";
import DischargeReceipt from "./discharge-receipt.vue";
import LoadingForm from "./loading-form.vue";

/** 新增弹窗（装船清单表单，本轮无接口） */
const [FormModal, formModalApi] = useVbenModal({
  connectedComponent: LoadingForm,
  destroyOnClose: true,
});

/** 查看回执弹窗（复用卸船清单回执，本轮无接口） */
...
/** 查看回执 */
function handleViewReceipt(row: LoadingManifest) {
  receiptModalApi.setData(row).open();
}

/** 删除装船清单 */
function handleDelete(row: LoadingManifest) {
  todo(`删除(${row.shipCallSign})`);
}

/** 装船清单表格（本轮使用本地假数据，暂不接入分页查询接口） */
const [Grid] = useVbenVxeGrid({
  formOptions: {
    schema: useLoadingGridFormSchema(),
    submitButtonOptions: {
      content: "查询",
    },
  ...
  } as VxeTableGridOptions<LoadingManifest>,
});
</script>

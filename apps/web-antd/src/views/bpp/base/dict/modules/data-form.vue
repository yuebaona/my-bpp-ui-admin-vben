...
import type { BaseDictDataApi } from '#/api/bpp/base/dict/data';
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    modalApi.lock();
    // 提交表单
    const data = (await formApi.getValues()) as BaseDictDataApi.DictData;
    try {
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data = modalApi.getData<BaseDictDataApi.DictData>();
    if (!data || !data.id) {
      // 设置 dictType

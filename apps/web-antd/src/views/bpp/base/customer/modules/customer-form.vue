...
const formData = ref<BppBaseCustomerApi.Customer>();
const getTitle = computed(() => {
  return formData.value?.id
    ? $t('ui.actionTitle.edit', ['客户基础信息'])
    : $t('ui.actionTitle.create', ['客户基础信息']);
});
...
const [Modal, modalApi] = useVbenModal({
  ...
  async onOpenChange(isOpen: boolean) {
    ...
    // 加载数据
    const data = modalApi.getData<BppBaseCustomerApi.Customer>();
    if (!data || !data.id) {
      return;
    }
    modalApi.lock();

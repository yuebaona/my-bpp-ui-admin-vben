...
② 在 `application.yaml`中，添加 `spring.cloud.config.import` 配置项。
...
按照需要，将不同环境存在差异的 `application-local.yaml`和 `application-dev.yaml`中的配置，迁移到 Nacos 配置中心。以 `application-local.yaml` 为例子。
...
对应的后端代码是 `bpp-module-infra` 的 `config`业务模块。
...
- `key` 字段，对应到 Spring Boot 配置文件的配置项，例如说 `bpp.captcha.enable`、`sys.user.init-password` 等等。
